...
export const OutgoingAccountInvitations = () => {
  ...
  const fetchInvitations = async () => {
    try {
      setLoading(true);
      const outgoingRes = await api.get("/teams/account-invitations/sent");
      if (outgoingRes.data.success) {
        setOutgoingInvitations(outgoingRes.data.data.invitations || []);
      }
    } catch (error) {
      console.error("Failed to fetch outgoing account invitations:", error);
      toast.error("Failed to fetch outgoing account invitations.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSendInvite = async () => {
    ...
    try {
      ...
      const response = await api.post("/teams/account-invitations/send", {
        email: inviteEmail,
        role: inviteRole,
        accountId,
      });
      ...
      console.error("Failed to send account invitation:", error);
      toast.error(
  ...
  const handleResend = async (invitationId) => {
    try {
      ...
      const response = await api.post("/teams/account-invitations/resend", {
        invitationId,
      });
      if (response.data.success) {
      ...
      console.error("Failed to resend account invitation:", error);
  ...
  const handleRevoke = async (invitationId) => {
    ...
    try {
      setProcessingId(invitationId);
      const response = await api.delete("/teams/account-invitations/revoke", {
        data: { invitationId },
      });
      if (response.data.success) {
      ...
      console.error("Failed to revoke account invitation:", error);
  ...
  const currentUserAccountLevel = ROLE_LEVEL[account?.role] ?? -1;
    ...
    <div className="flex h-full flex-col space-y-6 overflow-y-auto p-4 sm:p-6 lg:p-8">
      ...
      <div>
        ...
        <div className="mt-3 flex items-start gap-2 rounded-md border border-blue-100 bg-blue-50 px-3 py-2.5 text-xs text-blue-700">
          ...
          <span>
            Inviting someone to the account will automatically add them to{" "}
            <strong>all existing chatbots</strong> with the selected role.
            Any <strong>new chatbot</strong> you create will also include them with
            this role. You can still override their role for individual chatbots
            from the Chatbot Members page.
          </span>
        </div>
        <div className="mt-4 flex flex-col gap-3 xl:flex-row">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent className="w-[300px] p-2" align="end">
              {ROLES.map((role) => {
                const isDisabled =
                  ROLE_LEVEL[role.value] > currentUserAccountLevel;
                return (
                  <DropdownMenuItem
                    key={role.value}
                    disabled={isDisabled}
      ...
      {/* ── Outgoing Account Invitations ── */}
      <div className="flex-1">
        <h3 className="border-b border-gray-100 pb-2 text-sm font-semibold text-gray-600">
          Outgoing Account Invitations (Sent by you)
        </h3>
        ...
        {loading ? (
          ...
          <div className="py-8 text-center text-sm text-gray-400 italic">
            No outgoing account invitations.
          </div>
        ) : (
...
export const IncomingAccountInvitations = () => {
  const { user } = useAuth();
  ...
  const fetchInvitations = async () => {
    try {
      setLoading(true);
      const incomingRes = await api.get("/teams/account-invitations/incoming");
      ...
      console.error("Failed to fetch incoming account invitations:", error);
      toast.error("Failed to fetch incoming account invitations.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAccept = async (token, invitationId) => {
    try {
      setProcessingId(invitationId);
      const response = await api.post("/teams/account-invitations/accept", {
        inviteToken: token,
      });
      if (response.data.success) {
      ...
      console.error("Failed to accept account invitation:", error);
  ...
  return (
    <div className="flex h-full flex-col space-y-6 overflow-y-auto p-4 sm:p-6 lg:p-8">
      {/* ── Incoming Account Invitations ── */}
      <div className="flex-1">
        <h3 className="border-b border-gray-100 pb-2 text-sm font-semibold text-gray-600">
          Incoming Account Invitations (Received)
        </h3>
        ...
        {loading ? (
          ...
          <div className="py-8 text-center text-sm text-gray-400 italic">
            No incoming account invitations.
          </div>
        ) : (
          <div className="mt-2 divide-y divide-gray-50">
