...
function AuthorBlock({ author }) {
  ...
  return (
    <div className="flex flex-col items-center text-center">
      <Link href={`/blog/author/${author.slug}`} className="group flex flex-col items-center text-center">
        ...
        <p className="mt-3 font-semibold text-xl text-slate-900 group-hover:text-blue-700">
          {author.name}
        </p>
        {author.title && <span className="mt-0.5 text-sm text-slate-500">{author.title}</span>}
        {author.bio && (
          <span className="mt-2 text-sm leading-relaxed text-slate-500">{author.bio}</span>
        )}
      </Link>
